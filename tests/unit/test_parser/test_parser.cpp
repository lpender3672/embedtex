...
static void test_mathit_single_token() { OK(U"\\mathit x", "x"       /* italic is the default for a letter, so unmarked */); }
...
static void test_nested_style_restores() {
  ...
  OK(U"\\mathbf{a\\mathit{b}c}", "[a!b b c!b]"  /* the middle letter is default italic */);
}
