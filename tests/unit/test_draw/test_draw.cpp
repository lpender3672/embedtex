...
static void test_missing_glyph_refuses_the_render() {
  // Inverted deliberately. This test used to assert that a CJK char "occupies
  // space but emits no coverage" -- which was defect GLYPH-COVERAGE encoded as
  // an expectation: render() returned Ok while silently dropping a character.
  // STX-FNT-05 says an unavailable (face, glyph) pair refuses, so layout now
  // fails and nothing is drawn.
  Arena a(g_buf, sizeof(g_buf));
  RecordingGraphics<512> rec;
  TEST_ASSERT_FALSE(render(a, rec, U"x中x", 3));
...
int main(int, char**) {
  ...
  RUN_TEST(test_missing_glyph_refuses_the_render);
